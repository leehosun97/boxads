...
<%
	...
	try{
		...
		query.append(" SELECT count(*) cnt from tbl_member M LEFT JOIN tbl_cod C using(member_idx) ");
		query.append(where);
		...
		query.append(" SELECT M.member_idx, aes_decrypt(unhex(M.member_email),'" + dbconn.getHashKey() + "') as member_email ");
		query.append(" ,CASE WHEN M.member_path='G' THEN '구글' WHEN M.member_path='F' THEN '페이스북' ELSE '이메일' END as member_path ");
		query.append(" ,M.member_registdate ,C.cod_phoneNumber ,member_path ,C.cod_name, CONCAT(M.member_last_name,M.member_name) as member_name ,C.cod_isconfirm " );
				...
				<div class="content">
					...
					<div class="list">
						...
						<div class="btn_area align_r mt20">
							<button  type="button" onclick="excel_download(this,'enterprise_list_excel');" class="btn btn_down">엑셀다운로드</button>
									...
									<c:choose>
										...
										<c:otherwise>
											<c:forEach items="${ list }" var="c" varStatus="i">	
												<tr>
													<td><input type="checkbox" name="RowCheck" value="${c.member_idx}"></td>
													<td>${page_start_num - i.index}</td>
													<td><a href="enterprise_view.jsp?member_idx=${c.member_idx}">${c.member_name}</a></td>
													<td><a href="enterprise_view.jsp?member_idx=${c.member_idx}">${c.cod_name}</a></td>
													<td>${c.member_email}</td>
													<td>${c.cod_phonenumber}</td>
													<td>${c.member_path}</td>
													<td><fmt:parseDate value="${c.member_registdate}" var="indate" pattern="yyyy-MM-dd"/>
														<fmt:formatDate value="${indate}" pattern="yyyy-MM-dd"/></td>
													<td>${c.member_use}</td>
													<td>
													<select onchange="isConfirm(this, ${c.member_idx})">
														<c:choose>
															<c:when test="${c.cod_isconfirm eq 'Y'}">
